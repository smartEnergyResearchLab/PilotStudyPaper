---
title: "SERL response/consent model testing"
subtitle: "Exploring D-W stat issues and testing results plotting methods"
author: "Ben Anderson"
...
Having established our base model we'll now add `incentive`, `p2w` and `version` to see which pathway made a difference.

```{r, complexModel}
m <- glm(formula = consent ~ as.factor(quintile) + incentive + p2w + as.factor(version) + 
                          region , 
                       family = binomial(logit), all_onboards) # will auto-drop the missing values
...
# Complex non-response model: adding model interactions

Having established our base model we'll now add an interaction to see if the consent rate of those in certain quintiles is influenced by the incentive.
...
df <- broom::tidy(m)
df$model <- "4. Interactions"

interactionModelDT <- data.table::as.data.table(cbind(coefCI, df)) # collect for later plot

mInteraction <- m
```

Interestingly we do not find a statistically significant effect for the interactions. This model also removes the main effect of the incentive suggesting that the incentive effect is relatively small and unstable.
...
plotDT <- rbind(shuffledModelDT, complexModelDT, interactionModelDT)
data.table::setnames(plotDT, c("2.5 %","97.5 %"), c("lo", "up")) # for easier ggplotting
...
```
We really need to re-name the variable labels to make sense of the plot as the variables we might want to compare are not next to each other. However, the plot confirms the previous conclusions on the effect of IMD and region on incentive. We can also see that the interaction effects have larger 95% CIs - this explains why the main effects for incentives are statistically significant but the interaction effects are not. You should be able to see that the main effects coefficients for `Voucher` are actually the same for the main effects in each model.

This is easier to see if we have the model results side by side in a table. Stargazer can do that too... and it saves this table for us as well.

We can now see that the `incentiveThermometer` main effect is unstable but the `incentiveVoucher` remains unchanged when interactions between IMD and incentive are added.

```{r, allModelOutput, results='asis'}
stargazer::stargazer(mShuffled, mComplex, mInteraction,
                     type = "html", 
                     title="Shuffled Model", digits=3, 
                     out=here::here("outputTables","allModels.txt"))
```

Next we  test if the model with all the co-variates (but not the interactions) is an improvement using `anova` with the Chisq test method (as our models are logistic):
...
Finally we can test if the model with all the co-variates and the interactions is an improvement on the no-interactions using `anova` with the Chisq test method (as our models are logistic):

```{r, testModelsInt}
anova(mComplex, mInteraction, test = "Chisq")

```

Nope - and that is what we would suspect given the lack of statistically significant interaction effects.
